public class Matching {
    ...
    public String findBestAnswer() {
        ...
        // Special case: company name question
        if ((hasWord("company") || hasWord("companys")) && hasWord("name")) {
            for (String candidate : candidates) {
                if (candidate.toLowerCase().contains("inficon")) {
                    return extractBestLine(candidate);
                }
            }
        }
...
// blockの中から最適な行を選ぶ
    ...
    private String extractBestLine(String block) {
        ...
        if ((hasWord("company") || hasWord("companys")) && hasWord("name")) {
            String cleaned = block.replace("Company Overview", "").trim();
    ...
    private int calculateScore(SimpleMap<String, Integer> questionWords, String candidateLine) {
        ...
        for (String word : candidateWords) {
            String cleanWord = word.toLowerCase().replaceAll("[^a-z0-9]","").trim();
...
//                   }
    ...
    private boolean hasWord(String word) {
        return questionScraper.wordFrequencyMap.containsKey(word);
    }
